...
<section id="privacy" class="container">
    <div>
        ...
        <p class="bold">{{ __('This information is held by') }} {{ env('APP_NAME', 'Badminton.io') }} {{ __('as the "Controller" for the purposes of') }}:</p>
        <ul>
        ...
        <p class="bold">{{ __('This data is passed to the following external systems') }}:</p>
        <ul>
        ...
        <p>{{ __('We do not pass this information to anyone else for marketing or any other purposes, so there is not an opt-in for any additional options.') }}</p>
        ...
        <h3>{{ __('Leagues use') }} {{ env('APP_NAME', 'Badminton.io') }} {{ __('to manage their day to day activities including:') }}</h3>
        <ul>
            <li>{{ __('Scheduling') }}</li>
        ...
        <h3>{{ __('Minimum personal data held within') }} {{ env('APP_NAME', 'Badminton.io') }} {{ __('about league members in the league:') }}</h3>
        <ul>
            <li>{{ __('Name') }}</li>
        </ul>
        ...
        <p class="bold">{{ __('Right to rectification') }}</p>
        <p>{{ __('All data relating to an individual can be corrected by any League Administrator within a league.') }}</p>

        <p class="bold">{{ __('Right to erasure') }}</p>
        <p>{{ __('We provide the tools to completely erase a person in your league. There is no way to revert this. All personal data will be deleted and the player or person will have a first name and last name anonymized.') }}</p>

        <p class="bold">{{ __('Right to restrict process') }}</p>
        <p>{{ __('We provide the tools to restrict the processing of an individual. Their personal details will not be viewable within the administration system or the public sites and their first name and last name are anonymized.') }}</p>

        <h2 class="h24px">{{ __('Where we store your personal data') }}</h2>
        <p>{{ __('All') }} {{ env('APP_NAME', 'Badminton.io') }} {{ __('data is stored using') }} "<a href="https://aws.amazon.com/">Amazon Web Services</a> (AWS)", {{ __('they are a subsidiary of Amazon.com that provides a cloud computing platform for') }} {{ env('APP_NAME', 'Badminton.io') }}. <a href="https://aws.amazon.com/privacy/">AWS {{ __('Privacy') }}</a></p>
        <p>{{ __('All passwords are encrypted. The administration pages all operate under https, this means all communications between your browser and the website are encrypted.') }}</p>
        ...
        <h3>{{ __('Category 3 - Functionality Cookies') }}</h3>
        <p class="bold">{{ __('These cookies remember choices you make to improve your experience.') }}</p>
        <p>{{ __('We do not use this type of cookie directly on our website.') }}</p>
        ...
        <p>{{ __('Any changes we may make to our privacy policy in the future will be posted on this page. This policy was last updated: ') }}<strong>{{ __('26 September 2022') }}</strong></p>

        <h2 class="h24px">Contact</h2>
        <p>{{ __('Questions, comments and requests regarding this privacy policy should be addressed to') }} {{ env('APP_NAME', 'Badminton.io') }}, DJH Technology Ltd, Granville House, 2 Tettenhall Rd, Wolverhampton WV1 4SB, United Kingdom or <a class="lowercase" href="#" onclick="Intercom('showNewMessage');return false;">Contact Us</a></p>
