                            ...
                            <div class="card league-card">
                                ...
                                <div class="card-header league-banner"
                                    ...
                                    @if (now() < date('Y-m-d', strtotime($league->end_date_register)))
                                        <div class="label lb-register">
                                            <span class="extend_lb label-success">{{__('Registering')}}</span>
                                        </div>
                                    @elseif(now() > date('Y-m-d', strtotime($league->end_date_register)) &&
                                            now() < date('Y-m-d', strtotime($league->start_date)))
                                        <div class="label lb-register">
                                            <span class="extend_lb label-end">{{__('End Register')}}</span>
                                        </div>
                                    @endif
                                ...
                                <div class="card-body text-center">
                                    ...
                                    <p class="league-info p-0">
                                        {{ $league->format_of_league }} || {{ $league->type_of_league }} ||
                                        {{ $start_date }} || {{ $end_date }}
                                        || {{ $league->location }}
                                    </p>
                                    ...
                                    <!-- Thống kê -->
                                    <div class="league-stats">
                                        <span><i class="fas fa-users"></i> {{ count($league->userLeagues) }} /
                                            {{ $league->number_of_athletes }} </span>
                                    </div>
