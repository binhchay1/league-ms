        ...
        <div class="">
            ...
            <div class=" text-black align-items-center" style="background: #707787;padding: 10px; margin-top: -20px;">
                <div class="container d-flex p-0 img-fluid">
                    <img src="{{ asset(Auth::user()->profile_photo_path ?? '/images/no-image.png') }}" alt="User"
                        width="150" height="150" class=" me-3 ">
                    <div>
                        ...
                        <p class="mb-1 text-muted ">
                            <i class="bi bi-telephone text-white"></i> <em
                                class="text-white">{{ Auth::user()->phone ?? 'updating' }}</em>
                        </p>
                        <p class="mb-0 text-muted ">
                            <i class="bi bi-calendar text-white"></i> <em
                                class="text-white">{{ Auth::user()->age ?? 'updating' }}</em>
                        </p>
                    </div>
                </div>
            </div>
                            ...
                            @foreach ($listLeague as $row)
                                @if ($row && isset($row->slug))
                                    <div class="card mb-3">
                                        <div class="row g-0">
                                            <div class="col-md-2">
                                                <img src="{{ asset($row->images ?? asset('/images/no-image.png')) }}"
                                                    class="img-fluid rounded-start" alt="BattleBots">
                                            </div>
                                            <div class="col-md-10">
                                                <div class="card-body">
                                                    ...
                                                    @elseif(now() > date('Y-m-d', strtotime($row->end_date_register)) && now() < $row->start_date)
                                                        <span id="end-reg"
                                                            class=" status-league p-1 bg-warning text-white rounded">{{ 'End Register' }}</span>
