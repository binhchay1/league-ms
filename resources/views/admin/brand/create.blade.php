...
<div class="container-fluid mt-4">
    <div class="card card-default">
        ...
        <div class="card-body">
            <form id="formAccountSettings" method="POST" action="{{ route('brand.store') }}" enctype="multipart/form-data">
                @csrf()
                    <div class="col-md-6">
                        <div class="row mt-4">
                            <div class="col-12">
                                <label for="lastName" class="form-label">{{__('Name')}}</label>
                                <input class="form-control" value="{{ old('name') }}" type="text" name="name" id="name" placeholder="{{ __('Enter brand name') }}"/>
                                @if ($errors->has('name'))
